...
// ========== جميع المسارات تتطلب مصادقة ودور مندوب ==========
router.use(auth);
router.use(driverMiddleware);

// ========== 1. الملف الشخصي والحالة ==========
router.get('/profile', driverController.getMyProfile);
router.get('/status', driverController.getMyDetailedStatus);  // ✅ جديد: حالة المندوب التفصيلية
...
// ✅ فصل الحالتين: 
router.put('/availability', driverController.toggleAvailability);  // تبديل التوفر فقط (isAvailable)
router.put('/online', driverController.toggleOnline);              // ✅ جديد: تبديل الاتصال فقط (isOnline)
